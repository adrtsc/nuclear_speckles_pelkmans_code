...
def make_regression(df):
    ...
    for i in range(0, 10):
        ...
        prediction_list.append(pd.DataFrame(({'iteration': i,
                                       'log2FoldChange': y_test,
    ...
    return cv_result, prediction_df
...
def main():
    ...
    # run MLR
    cv_result, prediction_df = make_regression(df)

    cv_result.to_csv(output_path.joinpath('20230816_cv_result.csv'))
    prediction_df.to_csv(output_path.joinpath('20230816_predictions.csv'))
